...
export default function ImageViewer({
  ...
  }, [visible, initialIndex, overlayOpacity, dragTranslateY, enterScale, enterOpacity, exitScale, exitOpacity, reduceMotion]);
  ...
  const topBarAnimStyle = useAnimatedStyle(() => {
    // 拖拽启动时顶栏/缩略条随进度淡出（Twitter 行为）
    const dragFade = Math.min(Math.abs(dragTranslateY.value) / 140, 1);
    return { opacity: overlayOpacity.value * (1 - dragFade) };
  });
  ...
  const bgScrimStyle = useAnimatedStyle(() => {
    ...
    return { opacity: (1 - reveal) * enterOpacity.value * exitOpacity.value };
  });

  const bgBlurStyle = useAnimatedStyle(() => {
    const reveal = Math.min(Math.abs(dragTranslateY.value) / BG_REVEAL_DISTANCE, 1);
    return { opacity: reveal * enterOpacity.value * exitOpacity.value };
  });

  /**
   * 内容统一动效：入场缩放 × 拖拽位移/缩小（1→0.72，320pt 全程）× 退场缩放。
   * 单一 transform 计算避免多 style 数组 transform 相互覆盖。
   * 拖拽时同步增大圆角（Twitter 缩小图片的圆角收束感）。
   */
  const contentStyle = useAnimatedStyle(() => {
    const dragProgress = Math.min(Math.abs(dragTranslateY.value) / SHRINK_DISTANCE, 1);
    const scale = enterScale.value * (1 - dragProgress * 0.28) * exitScale.value;
    const dragFade = Math.min(Math.abs(dragTranslateY.value) / (SCREEN_HEIGHT * 0.6), 0.25);
    return {
      transform: [{ translateY: dragTranslateY.value }, { scale }],
  ...
  // Twitter-style drag-to-dismiss. The pan only activates on vertical
  ...
  const dismissGesture = Gesture.Pan()
    ...
    .onEnd((e) => {
      if (e.translationY > 140 || e.velocityY > 900) {
        // 沿拖拽方向飞出（背景揭示到 100%，图片缩小淡出），结束后卸载
